...
template <class T> class FContinueAction : public FPendingLatentAction
{
 ...
 FContinueAction(FWeakObjectPtr RequestObj, T& ResultParam, const FLatentActionInfo& LatentInfo, const TSharedPtr<FFrame> &Frame = TSharedPtr<FFrame>()) :
  Called(false)
  , CallFrame(Frame)
    , Result(ResultParam)
  ...
  {
    Response.FinishAndTriggerIf(Called, ExecutionFunction, OutputLink, CallbackTarget, CallFrame);
  }
